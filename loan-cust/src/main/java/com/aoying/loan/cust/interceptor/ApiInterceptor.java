...
public class ApiInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		logger.info(">>>>>>>>>> API IP {} 请求 {}?{}", reqIP, reqURI, request.getQueryString());
		...
		while (enu.hasMoreElements()) {
			...
			if ("content-type".equalsIgnoreCase(headerName) ||
					"account".equalsIgnoreCase(headerName) ||
					"secret".equalsIgnoreCase(headerName) ||
					"timestamp".equalsIgnoreCase(headerName) ||
					"sign".equalsIgnoreCase(headerName) ||
					"x_usertoken".equalsIgnoreCase(headerName)) {
		...
		// 校验签名
		if (isRelease && reqURI.contains("/api/")) {
		...
		if (reqURI.contains("Pro/")) {
			...
			logger.info(">>>>>>>>>> 用户编号 {} 用户手机 {}", cust.getId(), cust.getTelNo());
		}
		return true;
	}
