...
public class HttpClientTool {
	...
	public static void main(String[] args) throws Exception {
		...
		work = true;
		if (work) {
			String url = "https://172.16.24.246:60443/api?method=get.token&username=serve&password=kyd123";
			// String result = requestGet(getSSLContext(), url, null, "UTF-8", false);
			// System.out.println(result);
			String result = get(getSSLContext(), url);
			System.out.println(result);
		}
		work = false;
		if (work) {
			String url = "https://139.196.149.142:8443/loan/custLoanInfo/api/getCustLoanInfoPro/v1";
			Map<String, String> headers = new HashMap<String, String>();
			headers.put("X_ClientToken", "7CC5CF50E05529752BEFD677E8A51A06");
			headers.put("X_UserToken", "8E13013DBA0FF0E004393BA7AFAE8A40");
			Map<String, String> parames = new HashMap<String, String>();
			// parames.put("loanProductId", "1");
			// parames.put("custId", "1");
			// String result=HttpTool.requestPost(null, url, headers, parames);
			// SSLContext sSLContext =null;
			SSLContext sSLContext = getSSLContext("c:\\kyloanClient.keystore", "bdxiaodai-kyloanClient");
			String result = postParames(sSLContext, null, url, headers, parames);
			System.out.println(result);
		}
		work = false;
		if (work) {
			String url = "http://127.0.0.1:9080/loan/feedback/api/submitFeedbackPub/v1";
			Map<String, String> headers = new HashMap<String, String>();
			headers.put("X_ClientToken", "7CC5CF50E05529752BEFD677E8A51A06");
			headers.put("X_UserToken", "8E13013DBA0FF0E004393BA7AFAE8A40");
			Map<String, String> parames = new HashMap<String, String>();
			parames.put("contactWay", "13000000000");
			parames.put("content", "反馈测试");
			Map<String, List<File>> files = new HashMap<String, List<File>>();
			List<File> fs = new ArrayList<File>();
			for (int i = 1; i <= 3; i++) {
				File f = new File("c:/" + i + ".png");
				fs.add(f);
			}
			files.put("file", fs);
			try {
				String result = postFile(getSSLContext(), url, headers, parames, files);
				System.out.println(result);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		...
		if (work) {
			try {
				String url = "http://api.map.baidu.com/geocoder/v2/?ak=0VXHcpKiTkwWGXlttAjbnpmd&location=39.983424,116.322987&output=json";
		...
		work = false;
		if (work) {
			String url = "https://me.baodou365.com:8443/kyloanserver/cust/api/loginPub/v1";
			Map<String, String> headers = new HashMap<String, String>();
			headers.put("X_ClientToken", "7CC5CF50E05529752BEFD677E8A51A06");
			headers.put("custToken", "wangjz");
			Map<String, String> parames = new HashMap<String, String>();
			parames.put("telNo", "13000000000");
			parames.put("loginPwd", "111111");
			Map<String, File> files = new HashMap<String, File>();
			File f = new File("c:/1.avi");
			System.out.println(f.exists());
			files.put("file", f);

			try {
				String result = postParames(getSSLContext(), url, headers, parames);
				System.out.println(result);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
