...
public class StringTool {
	...
	/**
	 * 用*号隐藏身份证号码，只显示前2位和后4位
	 * @param idCard
	 * @return
	 */
	public static String hiddenIdCardCode24(String idCard) {
	...
	/**
	 * 用*号隐藏身份证号码，只显示前4位和后4位
	 * @param idCard
	 * @return
	 */
	public static String hiddenIdCardCode44(String idCard) {
		if (StringUtils.isEmpty(idCard)) {
			return null;
		}
	...
	/**
	 * 用*号隐藏身份证号码，只显示前4位和后2位
	 * @param idCard
	 * @return
	 */
	public static String hiddenIdCardCode42(String idCard) {
		if (StringUtils.isEmpty(idCard)) {
			return null;
		}
		return idCard.replaceAll("^(\\w{4})(\\w*)(\\w{2})$", "$1************$3");
	}
	...
	public static String hiddenIdCardName(String idCardName) {
		if (StringUtils.isEmpty(idCardName)) {
			...
			if (idCardName.length() > 3) {
				for (int i = name.length(); i < idCardName.length()-2; i++) {
					name.append("*");
				}
				name.append(idCardName.substring(idCardName.length()-2, idCardName.length()));
			} else {
				for (int i = name.length(); i < idCardName.length()-1; i++) {
					name.append("*");
				}
				name.append(idCardName.substring(idCardName.length()-1, idCardName.length()));
			}
	 ...
	 * @return
	 */
	public static String hiddenIdCardName2(String idCardName) {
		if (StringUtils.isEmpty(idCardName)) {
			return null;
		} else {
			StringBuffer name = new StringBuffer();
			if (idCardName.length() > 3 && idCardName.length() <= 5) {
				name.append("**");
			}else if(idCardName.length() > 5) {
				name.append("****");
			}else{
				name.append("*");
			}
			for (int i = name.length(); i < idCardName.length(); i++) {
				name.append(idCardName.charAt(i));
			}
			return name.toString();
		}
	}
	...
	public static String getSerno() {
		...
		return "QDBDXD" + format.format(new Date()) + "00" + (random.nextInt(max) % (max - min + 1) + min);
	}
