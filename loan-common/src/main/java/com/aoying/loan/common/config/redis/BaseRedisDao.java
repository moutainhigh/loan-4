package com.aoying.loan.common.config.redis;
...
public class BaseRedisDao {
	...
	public <T> List<T> getList(String key, Class<T> cls) {
		return getList(key, cls, 0);
	}

	/**
	 * 根据key取得列表,同时更新列表的失效时间,expSecond>0时才设置失效时间
	 *
	 * @param key
	 * @param cls
	 * @param expSecond 单位秒
	 * @return
	 */
	public <T> List<T> getList(String key, Class<T> cls, int expSecond) {
		ShardedJedis jedis = null;
		try {
			jedis = shardedJedisPool.getResource();
			String str = jedis.get(key);
			if (str != null) {
				...
				return JsonTool.getList(str, cls);
			}
			return null;
		} finally {
			if (jedis != null) {
